 ...
 */
export type FunctionCallArgValueID = string & {__FunctionCallArgValueID: never}
...
export class Container extends BaseClient {
  ...
  export = async (path: string, 
    opts?: ContainerExportOpts): Promise<string> => {
    if (this._export) {
      return this._export
    }
   ...
   * Defaults to "OCI", which is compatible with most recent registries, but "Docker" may be needed for older registries without OCI support.
   */
  publish = async (address: string, 
    opts?: ContainerPublishOpts): Promise<string> => {
    if (this._publish) {
    ...
    const ctx = this._ctx.select(
      "publish",
      { address, ...opts, __metadata: metadata},
    )
   ...
   */
  up = async (
    opts?: ContainerUpOpts): Promise<void> => {
    if (this._up) {
      return
    }
  ...
  withMountedCache = (path: string, cache: CacheVolume, opts?: ContainerWithMountedCacheOpts): Container => {
    ...
    const ctx = this._ctx.select(
      "withMountedCache",
      { path, cache, ...opts, __metadata: metadata },
    )
    return new Container(ctx)
  }
...
export class Directory extends BaseClient {
   ...
   */
  entries = async (
    opts?: DirectoryEntriesOpts): Promise<string[]> => {
    const ctx = this._ctx.select(
      "entries",
      { ...opts},
    )
   ...
   */
  export = async (path: string, 
    opts?: DirectoryExportOpts): Promise<string> => {
    if (this._export) {
      return this._export
    }
...
export class File extends BaseClient {
   ...
   */
  digest = async (
    opts?: FileDigestOpts): Promise<string> => {
    if (this._digest) {
      return this._digest
    }
   ...
   */
  export = async (path: string, 
    opts?: FileExportOpts): Promise<string> => {
    if (this._export) {
      return this._export
    }
...
export class GitRepository extends BaseClient {
   ...
   */
  branches = async (
    opts?: GitRepositoryBranchesOpts): Promise<string[]> => {
    const ctx = this._ctx.select(
      "branches",
      { ...opts},
    )
   ...
   */
  tags = async (
    opts?: GitRepositoryTagsOpts): Promise<string[]> => {
    const ctx = this._ctx.select(
      "tags",
      { ...opts},
    )
...
export class Module_ extends BaseClient {
   ...
   */
  serve = async (
    opts?: ModuleServeOpts): Promise<void> => {
    if (this._serve) {
      return
    }
...
export class Client extends BaseClient {
  ...
  moduleSource = (refString: string, opts?: ClientModuleSourceOpts): ModuleSource => {
    ...
    const ctx = this._ctx.select(
      "moduleSource",
      { refString, ...opts, __metadata: metadata },
    )
    return new ModuleSource(ctx)
  }
...
export class Service extends BaseClient {
   ...
   */
  endpoint = async (
    opts?: ServiceEndpointOpts): Promise<string> => {
    if (this._endpoint) {
   ...
   */
  stop = async (
    opts?: ServiceStopOpts): Promise<Service> => {
    const ctx = this._ctx.select(
      "stop",
      { ...opts},
    )
   ...
   */
  up = async (
    opts?: ServiceUpOpts): Promise<void> => {
    if (this._up) {
      return
    }
