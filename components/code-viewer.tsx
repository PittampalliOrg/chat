...
export function CodeViewer({ initialFiles, defaultSelectedFile, className }: CodeViewerProps) {
  const [sidebarOpen, setSidebarOpen] = React.useState(true)
  const [mobileOpen, setMobileOpen] = React.useState(false)
  const isDesktop = useMediaQuery("(min-width: 768px)")

  const { files, selectedFilePath, expandedDirs, toggleDirectory, selectFile, getSelectedFile } =
  ...
  return (
    <div className={cn("flex h-full overflow-hidden rounded-lg border", className)}>
      ...
      <div className="flex flex-1 flex-col overflow-hidden">
        ...
        <CodePane file={selectedFile} className="flex-1" />
      </div>
    </div>
  )
}
