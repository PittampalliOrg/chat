...
ensure_blob_contrib() {
 # If we are logged-in with a user use /me, otherwise fall back to the SP objectId
 if az account show --query user.type -o tsv 2>/dev/null | grep -q "user"; then
     me=$(az ad signed-in-user show --query id -o tsv)
 else
     sp_appid=$(az account show --query user.name -o tsv)  # this is the clientId
     me=$(az ad sp show --id "$sp_appid" --query id -o tsv)
 fi
...
###############################################################################
create_kind_cluster() {
  _require_storage_account
  local issuer
  issuer=$(az storage account show -n "$AZURE_STORAGE_ACCOUNT" -o json | jq -r '.primaryEndpoints.web')
  log "☸️  (re)creating Kind cluster '$KIND_CLUSTER_NAME' with SA issuer $issuer"
...
ensure_radius_app_registration() {
  # Look up any existing app whose *display-name* matches.
  local existing_app_id object_id issuer
  existing_app_id=$(az ad app list \
        --display-name "$APP_NAME" --query '[0].appId' -o tsv 2>/dev/null) || true

  [[ -z "$existing_app_id" ]] && { log "ℹ️  No existing app – will create fresh"; return; }

  object_id=$(az ad app show --id "$existing_app_id" --query id -o tsv)
  issuer=$(az ad app federated-credential list --id "$object_id" \
               --query '[0].issuer' -o tsv 2>/dev/null || echo "")

  if [[ "$issuer" != "$SERVICE_ACCOUNT_ISSUER" ]]; then
    log "⚠️  App issuer drift ($issuer ≠ $SERVICE_ACCOUNT_ISSUER) – deleting"
    az ad sp delete --id "$existing_app_id" -o none || true
    az ad app delete --id "$existing_app_id" -o none || true
  else
    log "✅  Existing app matches issuer – keeping"
  fi
}
...
render_infra_secrets() {
  ...
  source "${SCRIPT_PATH}/render-deployments.sh"   # already writes to OUT_DIR
}
...
create_eso_service_account() {
  ...
  APP_ID="$current_app_id"   # ← keep global var in sync
...
DEPLOY_DIR="${SCRIPT_PATH}/../deployments"

apply_deployments() {
  log "📦  kubectl apply manifests in $DEPLOY_DIR"
  find "$DEPLOY_DIR" -type f \( -name '*.yaml' -o -name '*.yml' -o -name '*.yaml.tpl' -o -name '*.yml.tpl' \) |
